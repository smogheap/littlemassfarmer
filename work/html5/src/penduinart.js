...
penduinART = {
	...
	load: function load(js) {
		var data = js;
		var name = data.name || ("anonymous" + penduinART.uniq++);
		penduinART.json[name] = data;
		return name;
	},
	list: function list() {
		return Object.keys(penduinART.json);
	}
};
...
function penduinOBJ(obj, cb) {
	...
	var drawPart = function drawPart(ctx, part, scale, displayx, displayy, rot,
		...
		if(part.image) {
			...
			var skip = false;
			if(part.wipe) {
				if(typeof part.wipe.x === "number") {
					part.wipe.x = Math.min(part.wipe.x, 1);
					right = part.wipe.x * img.width;
					skip = (part.wipe.x === 0);
				}
				if(typeof part.wipe.y === "number") {
					part.wipe.y = Math.min(part.wipe.y, 1);
					bottom = part.wipe.y * img.height;
					skip = (part.wipe.y === 0);
				}
			}
			if(!skip) {
...
function penduinSCENE(screenCanvas, logicWidth, logicHeight,
	...
	this.resize = function resize() {
		...
		var parent = screenCanvas.parentElement;
...
*/
		...
		ctx.restore();
		screenCtx.drawImage(canvas, 0, 0, canvas.width, canvas.height);
		lastFrame = time;
	};
